// main-bundled.js - 开箱即用版主进程
// 与 main.js 逻辑一致，但使用打包的 Node.js 运行时直接运行 DeepSeek Harness，
// 完全离线，无需用户安装 Node.js，也不依赖 npx。
...
// ---------- 打包运行时路径 ----------
// 开发环境: <项目>/runtime/node/node.exe
// 打包环境: <resources>/runtime/node/node.exe
...
function getBundledNodePath() {
  return path.join(getRuntimeRoot(), 'node', isWindows ? 'node.exe' : 'node');
}

function getDshEntryPath() {
  return path.join(getRuntimeRoot(), 'node_modules', '@deepseek-ai', 'dsh', 'lib', 'bin.js');
}
...
// 启动 DeepSeek Harness（使用打包的 Node.js 直接运行，完全离线）
function handleStart(event) {
  ...
  if (!fs.existsSync(nodePath)) {
    event.reply('log', `错误: 未找到打包的 Node.js: ${nodePath}\n`);
    event.reply('status-change', 'stopped');
    return;
  }
  if (!fs.existsSync(dshEntry)) {
    event.reply('log', `错误: 未找到 DeepSeek Harness: ${dshEntry}\n`);
    event.reply('status-change', 'stopped');
    return;
  }
  ...
  killPortProcess(SERVER_PORT, () => {
    ...
    event.reply('log', `使用内置运行时: ${nodePath}\n`);

    harnessProcess = spawn(nodePath, [dshEntry, 'web'], {
      // 使用用户主目录作为工作目录，避免安装目录（如 Program Files）只读导致 dsh 无法写配置
      cwd: os.homedir(),
      shell: false,
      windowsHide: true,
      ...
      env: {
        ...process.env,
        // 让 dsh 及其子进程优先使用内置 Node/npm
        PATH: path.join(getRuntimeRoot(), 'node') + path.delimiter + (process.env.PATH || '')
      }
    });
